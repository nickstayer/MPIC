...
public class Worker(ApiService apiService, DbService dbService, ILogger logger)
{
    ...
    public async Task<int> UpdateDealsInDb(DateTime lastUpdateDateTime = default)
    {
        ...
        logger.LogInformation($"Метод: сверка активных в бд с апи");
        var activeDealsFromDb = await dbService.GetAndMapActiveDeals();
        ...
        logger.LogInformation($"Метод: сверка активных в апи с бд");
        var activeDealsFromApi = await apiService.GetAndMapActiveDeals();
